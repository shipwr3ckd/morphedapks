...
def _get_target_asset(release: dict, ext: str, src: str, ver: str, gitlab: bool) -> dict:
    if gitlab:
        matches = [a for a in release.get("assets", {}).get("links", []) if a.get("url", "").endswith(f".{ext}")]
        non_dev = [a for a in matches if "-dev" not in a.get("name", "") and "-dev" not in a.get("url", "")]
    else:
...
def _fetch_single_asset(src: str, tag: str, ver: str, fprefix: str, ext: str, cl_dir: Path, net: NetworkManager) -> tuple[Path, str]:
    ...
    if gitlab:
        asset_url = asset.get("direct_asset_url") or asset["url"]
        asset_name = asset.get("name") or Path(asset_url).name
    else:
        asset_url = asset["url"]
        asset_name = asset["name"]

    file = cl_dir / asset_name

    for old_file in cl_dir.glob(f"*{fprefix}-*.{ext}"):
        if old_file.is_file() and not old_file.name.startswith("tmp."):
            old_file.unlink(missing_ok=True)

    pr(f"Getting '{asset_name}' from '{asset_url}'")
    ...
    changelog = f"> ⚙️ » {tag}: `{clean_src.split('/')[0]}/{asset_name}`  \n"
